    ...
    <div class="px-7 py-3 flex flex-col justify-between h-screen">
        <div x-data="{infoPerangkat: false}">
            ...
            <div class="text-2xl text-center font-bold  cursor-default">
                <p class="{{ ($witelData['deleted_at']) ? 'text-red-500' : 'text-blue-600' }}">Info Witel {{ $witelData['nama_witel'] }}</p>
            ...
            {{-- Top Section --}}
                <div class="flex justify-between px-5 mb-5 items-end">
                    <div class="flex flex-col font-semibold">
                        <table>
                            <tr>
                                <td class="pr-3">PIC Witel</td>
                                <td>:</td>
                                <td>{{ $witelData['users']['name'] }}</td>
                            </tr>
                            <tr>
                                <td class="pr-3">NIK PIC Witel</td>
                                <td>:</td>
                                <td>{{ ($witelData['users']['nik']) ? $witelData['users']['nik'] : '-' }}</td>
                            </tr>
                            <tr>
                                <td class="pr-3">No Telp PIC</td>
                                <td>:</td>
                                <td>{{ ($witelData['users']['no_telp']) ? $witelData['users']['no_telp'] : '-' }}</td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td>:</td>
                                <td class="font-semibold capitalize rounded-lg {{ ($witelData['deleted_at']) ? 'text-red-500' : 'text-blue-500' }}">{{ ($witelData['deleted_at']) ? 'Terhapus' : 'Aktif' }}</td>
                            </tr>
                        </table>
                    </div>
                    {{-- Search --}}
                        <div class="flex justify-between items-center space-x-6">
                            <div class="">
                                ...
                                <input wire:model.debounce.200="keyword" class=" focus:ring-4 outline-none focus:outline-none ring-blue-300 rounded-full pl-7 py-1 duration-150" type="text" placeholder="Cari Witel..." autocomplete="off">
                            </div>
                        </div>
                </div>
            {{-- End Top Section --}}
