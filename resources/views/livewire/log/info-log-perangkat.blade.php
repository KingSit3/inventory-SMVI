    ...
    <div x-data="{detailLog: false}" class="px-7 py-3 flex flex-col justify-between h-screen">
        <div>
            ...
            {{-- Top Section --}}
                <div class="flex justify-start px-5 mb-5">
                    <div class="flex flex-col font-semibold">
                        <table>
                            <tr>
                                <td class="pr-3">Status Perangkat</td>
                                <td>:</td>
                ...
                <table class="table-fixed w-full">
                    <thead>
                        <tr>
                            ...
                            <th class="w-1/6">Sn Perangkat</th>
                    ...
                    <tbody>
                        @forelse ($logPerangkat as $value)
                        <tr class="text-center items-center {{ ($loop->odd) ? "bg-indigo-100 bg-opacity-75" : "" }}">
                            <td class="py-2">{{ ($logPerangkat->firstItem()-1) + $loop->iteration }}</td>
                            <td><a class="hover:text-blue-500 duration-200 font-semibold" href="/logperangkat/{{ $value['id_perangkat'] }}">{{ $value['Perangkat']['sn_pengganti']}}</a></td>
                            ...
                            <td class="space-x-4 py-1 flex items-center justify-center">
                                <button @click="detailLog = true" wire:click="$emit('detailPerangkat', {{ $value['id'] }})" class="focus:outline-none" title="Info">
            ...
            class="z-50 fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start overflow-y-auto">
                <livewire:log.detail.detail-perangkat />
            </div>
        </div>
        <div>
            {{-- Pagination --}}
