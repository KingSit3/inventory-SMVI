        ...
        <div>
            ...
            {{-- Table --}}
                <table class="table-fixed w-full mt-5">
                    ...
                    <tbody>
                        @forelse ($logTipeSistem as $value)
                        <tr class="text-center items-center {{ ($loop->odd) ? "bg-indigo-100 bg-opacity-75" : "" }}">
                            <td class="py-2">{{ ($logTipeSistem->firstItem()-1) + $loop->iteration }}</td>
                            <td><a class="hover:text-blue-500 duration-200 font-semibold" href="/logtipesistem/{{ $value['id_sistem'] }}">{{ $value['tipesistem']['kode_sistem']}}</a></td>
                            ...
                            <td class="space-x-4 py-1 flex items-center justify-center">
                                <button @click="detailLog = true" wire:click="$emit('detailTipeSistem', {{ $value['id'] }})" class="focus:outline-none" title="Info">
            ...
            class="z-50 fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start overflow-y-auto">
                <livewire:log.detail.detail-tipe-sistem />
            </div>
        </div>
        <div>
            {{-- Pagination --}}
