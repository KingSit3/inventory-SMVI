    ...
    <div x-data="{detailLog: false}" class="px-7 py-3 flex flex-col justify-between h-screen">
        <div>
            ...
            <div class="text-2xl text-center font-bold  cursor-default">
                <p class="{{ ($logWitel['deleted_at']) ? 'text-red-500' : 'text-blue-600' }}">Info Log Witel {{ $logWitel['nama_Witel'] }}</p>
            </div>
                    ...
                    <div class="flex flex-col font-semibold">
                        <table>
                            <tr>
                                ...
                                <td>:</td>
                                <td>{{ $logData['nama_witel'] }}</td>
                            </tr>
                            <tr>
                                <td class="pr-3">Kode Witel</td>
                                <td>:</td>
                                <td>{{ $logData['kode_witel'] }}</td>
                            </tr>
                            <tr>
            ...
            {{-- Table --}}
                <table class="table-fixed w-full">
                    ...
                    <tbody>
                        @forelse ($logWitel as $value)
                        <tr class="text-center items-center {{ ($loop->odd) ? "bg-indigo-100 bg-opacity-75" : "" }}">
                            <td class="py-2">{{ ($logWitel->firstItem()-1) + $loop->iteration }}</td>
            ...
            class="z-50 fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start overflow-y-auto">
                <livewire:log.detail.detail-witel />
            </div>
        </div>
        <div>
            {{-- Pagination --}}
