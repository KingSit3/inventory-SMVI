            ...
            {{-- Top Section --}}
                <div class="flex justify-between px-5 mb-5">
                    {{-- Tambah Button --}}
                        <div>
                        @if (session('role') != 2)
                            <button @click="isOpen = true" wire:click="add" class="bg-blue-500 hover:shadow-md hover:bg-blue-700 px-3 py-2 rounded-xl text-white font-semibold duration-150">Tambah Tipe Sistem</button>
                        @endif
                        </div>
                    ...
                    {{-- Search --}}
                        <div class="flex justify-between items-center space-x-6">
                            <div class="">
                                ...
                                <input wire:model.debounce.200="keyword" class=" focus:ring-4 outline-none focus:outline-none ring-blue-300 rounded-full pl-7 py-1 duration-150" type="text" placeholder="Cari Tipe Sistem...">
                            </div>
                        </div>
                </div>
            {{-- End Top Section --}}
                ...
                <table class="table-fixed w-full">
                    <thead>
                        <tr>
                            ...
                            <th class="w-1/6">Kode Tipe Sistem</th>
                            @if (session('role') != 2)
                    ...
                    <tbody>
                        @forelse ($tipeSistem as $value)
                        <tr class="text-center items-center {{ ($loop->odd) ? "bg-indigo-100 bg-opacity-75" : "" }}">
                            <td class="py-2">{{ ($tipeSistem->firstItem()-1) + $loop->iteration }}</td>
                            <td>{{ ($value['kode_sistem']) ? $value['kode_sistem'] : '-' }}</td>

                            <td class="space-x-4 py-1 flex items-center justify-center">
                                <a href="/tipesistem/{{ $value['id'] }}" class="focus:outline-none" title="Info">
                    ...
                    <form wire:submit.prevent="{{ $submitType }}">
                        <div class="px-8 py-6">
                            ...
                            <div class="mt-4 space-y-1 mx-10 font-semibold">
                                <label for="kode" class="cursor-default">Kode Tipe Sistem</label>
